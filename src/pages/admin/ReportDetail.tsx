...
import { ArrowLeft, FileDown, Trash, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
const ReportDetail = () => {
  ...
  const { reports, updateReportStatus, deleteReport, isLoading } = useAppContext();
  ...
  const handleStatusUpdate = async () => {
    if (report && statusToUpdate) {
      await updateReportStatus(report.id, statusToUpdate);
    ...
    <div className="space-y-6">
      ...
      <AlertDialog open={showStatusDialog} onOpenChange={setShowStatusDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {statusToUpdate === "completed" && "Mark report as completed?"}
              {statusToUpdate === "rejected" && "Reject this report?"}
              {statusToUpdate === "pending" && "Mark report as pending?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {statusToUpdate === "completed" && "This will mark the report as approved and completed."}
              {statusToUpdate === "rejected" && "This will reject the report and notify the technician."}
              {statusToUpdate === "pending" && "This will change the report status back to pending."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
